    ...
    <div :key="key" class="margin-top">
      ...
      <div>
        <span v-permission="['admin', 'user']" class="permission-alert">
          两者
          <el-tag>admin</el-tag>和 <el-tag>user</el-tag>都可以看见这个
        </span>
        <el-tag v-permission="['admin', 'user']" class="permission-code" type="info" size="large">
    ...
    <div :key="'checkPermission' + key" class="margin-top">
      ...
      <el-tabs type="border-card" class="margin-top">
        ...
        <el-tab-pane v-if="checkPermission(['user'])" label="user">
          user 可以看见这个
          <el-tag class="permission-code" type="info"> v-if="checkPermission(['user'])" </el-tag>
        </el-tab-pane>
        <el-tab-pane v-if="checkPermission(['admin', 'user'])" label="admin 和 user">
          两者 admin 和 user 都可以看见这个
          <el-tag class="permission-code" type="info"> v-if="checkPermission(['admin', 'user'])" </el-tag>
        </el-tab-pane>
      </el-tabs>
    </div>
  </div>
